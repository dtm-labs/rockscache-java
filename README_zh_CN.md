...
1. git clone本项目，使用IDE打开其example目录所代表的工程，等待gradle下载所有依赖。

2. 使用Docker在本机构建一个redis服务，端口保持默认的6379

   > 如果想基于现有的redis运行此demo，请修改[application.yml](https://github.com/dtm-labs/rockscache-java/blob/main/example/src/main/resources/application.yml)中spring.redis相关配置。

3. 使用Docker在本机构建一个mysql服务，端口保持默认的3306

   > 如果想基于现有的mysql运行此demo，请修改[application.yml](https://github.com/dtm-labs/rockscache-java/blob/main/example/src/main/resources/application.yml)中spring.datasource相关配置。
...
1. 设置缓存中键的前缀
   ...
   例如，上例中，订单缓存的前缀为`order-`，那么id为`1`的订单，在redis中对应的键就是`order-1`
...
4. 调用setJavaLoader方法，表明，如果查看的数据在reids中不存在，该如何从数据库中加载。

   > 这里的`setJavaLoader`是针对Java开发者的，如果是kotlin开发者，请调用`setKtLoader`。

5. 如果数据访问行为返回`List<V>`，而非`Map<K, V>`，这个Lambda表达式表示如何从值对象获得键。

   > 如果数据访问行为返回`Map<K, V>`; 不再需要指定此规则。这时，为了编译通过，用户必须调用`setJavaLoader`方法的其他重载形式，而其他重载形式无此参数。对于某些根本没有id的对象，这些重载版会很有用。
...
public class OrderService {
    ...
    public void saveOrder(Order order) {
         ...
         * 将被修改的redis数据标记成已删除
         *
         * 这个例子仅仅示范如何使用tagAsDeleted，
         * 不示范如何应对tagAsDeleted的异常。
         *（更极端的案例，tagAsDeleted之前程序就挂了，没执行）
         *
         * 在实际项目中，请使用你喜欢的任何可靠消息技术，
         * 保证tagAsDeleted一定会被执行。
         *
         * DTM的二阶段消息就是一个理想的选择。
         */
...
- `toCache`：`fetch(K)`和`fetchAll(Collection<K>)`这两个数据查询方法没有明确指定数据型一致性要求，即`Consistency`。默认情况下，会采用缓存对象默认的一致性。

  > 注意：
  > 
  > 此方法并不会改表当前Cache的默认Consistency，而是创建一个新的Cache，除了默认的Consistency不同外，新旧Cache没有任何差异。

- `fetch(K)`: 按照当前缓存对象默认的一致性要求，查询一个数据

- `fetch(K, Consistency)`: 按照用户指定的的一致性要求，查询一个数据

- `fetchAll(Collection<K>)`: 按照当前缓存对象默认的一致性要求，查询多个数据

- `fetchAll(Collection<K>, Consistency)`: 按照用户指定的一致性要求，查询多个数据

- `tagAsDeleted`: 将一个数据标记成已删除

- `tagAllAsDeleted`: 将多个数据标记成已删除

- `lockOperator(K, String)`: 指定一个随机的lockId，获取一个数据的锁操作对象
  
  > 注意
  > 
  > 返回的对象支持两个方法：`lock`和`unlock`。
  > 当前方法返回时，**并未上锁**。必须调用返回对象的`lock`方法才会上锁。
  > 返回对象的`lock`需要指定最大等待时间。如在规定时间内不能抢到锁，抛出异常。
  > 由于分布式通信的不可靠性，一旦调用了返回对象的`lock`，无论成败，都需要调用其`unlock`，而且要使用可靠消息来保证这点。

- `lockAllOperator(Collection<K>, String)`: 指定一个随机的lockId，获取多个数据的锁操作对象
  
  > 注意事项同`lockOperator(K, String)`
...
- **TRY_STRONG**: 尝试强一致。
    ...
    此异常可用于让界面显示：“数据暂时不可用，请稍后重试”，从而引导界面去获取一致的数据。
...
2. 从`CacheClient`创建`Cache`时，可以覆盖步骤1中的设置
   ...
   > 如果缓存级的覆盖性配置未指定，继承全局配置。

3. 构建新的Cache，覆盖旧Cache配置
